import streamlit as st
import google.generativeai as genai

# 金庫（Secrets）から鍵を取り出す
if "GOOGLE_API_KEY" in st.secrets:
    # transport='rest' で古い通信エラーを物理的に回避します
    genai.configure(api_key=st.secrets["GOOGLE_API_KEY"], transport='rest')
else:
    st.error("金庫に GOOGLE_API_KEY が入っていません。Settings > Secretsを確認してください。")

# --- 監査・リスク・出口戦略：最強の指示書 ---
SYSTEM_PROMPT = """
あなたは、日本の精神科訪問看護における監査・実地指導対応のプロフェッショナルです。
1.主治医報告書 / 2.4要素記録 / 3.超精密GAF評価 / 4.リスクコード(SI,AA,HD,CD,SR) / 5.出口戦略
これらを一括で生成してください。
"""

st.set_page_config(page_title="Inhapi 統合支援システム", page_icon="🏥")
st.title("🏥 Inhapi 監査・出口戦略・リスク生成")

memo = st.text_area("訪問時のメモを貼り付けてください", height=200)

if st.button("✨ 監査・リスク分析を実行"):
    if not memo:
        st.warning("メモを入力してください。")
    else:
        with st.spinner("最新モデルで精密分析中..."):
            try:
                # 404エラーを回避する最新モデルの呼び出し
                model = genai.GenerativeModel('gemini-1.5-flash')
                response = model.generate_content(f"{SYSTEM_PROMPT}\n\n入力メモ:\n{memo}")
                st.success("分析が完了しました！")
                st.markdown(response.text)
            except Exception as e:
                st.error(f"技術的エラーが発生しました: {e}")
